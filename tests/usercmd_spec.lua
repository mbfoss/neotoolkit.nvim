...
describe("usercmd.split_args", function()
    ...
    describe("word boundaries", function()
        it("keeps a mid-argument quoted segment literally", function()
            assert.same({ 'a"b"c' }, split('a"b"c'))
            assert.same({ 'a"bc', 'de"f' }, split('a"bc de"f'))
        end)

        it("joins a bare tail to a leading quoted span", function()
            assert.same({ "ac" }, split('"a"c'))
            assert.same({ "textsuffix" }, split('"text"suffix'))
            assert.same({ "a bc" }, split('"a b"c'))
            assert.same({ "abc" }, split('"a"b"c"'))
        end)

        it("joins a trailing quoted span to a leading one", function()
            assert.same({ "a bc d" }, split('"a b"c" d"'))
        end)

        it("strips quotes when the span ends the argument", function()
            assert.same({ "a b c" }, split('"a b c"'))
            assert.same({ "a b", "c" }, split('"a b" c'))
        end)
    end)
    ...
    describe("empty and edge cases", function()
        ...
        it("keeps an unterminated quote literally and still splits on whitespace", function()
            assert.same({ '"a', "b" }, split('"a b'))
    ...
    describe("backslash escaping", function()
        ...
        it("does not process a backslash outside a quoted span", function()
            assert.same({ 'a\\"c' }, split('a\\"c'))
            assert.same({ 'a\\"', "b" }, split('a\\" b'))
            assert.same({ '\\"a', 'b\\"' }, split('\\"a b\\"'))
        end)
